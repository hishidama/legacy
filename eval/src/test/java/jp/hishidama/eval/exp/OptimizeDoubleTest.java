...
public class OptimizeDoubleTest {
	...
	/*
	 * 'jp.hishidama.eval.Expression.optimizeDouble()' のためのテスト・メソッド
	 */
	@Test
	public void testOptimize() {
		...
		testOpt("10 / 3 * 3", null, 10);
		testOpt("3 * 10 / 3", null, 10);
		testOpt("1+2+a", null, "3.0 + a");
	...
	public void testArray() {
		...
		map.put("a", a);
		testOpt("a[1]", map, 11);
		testOpt("a[3]=a[1]+a[2]", map, "a[3] = 33.0");
		testOpt("a[4]", map, "a[4]");
	...
	public void testLet() {
		...
		testOpt("a|=b|=255-15", map, "a |= b |= 240.0");
	...
	public void testFunction() {
		...
		testOpt("zzz()+foo(1)+test(2,3+4,5*6-7)", null,
				"zzz() + foo(1) + test(2, 7.0, 23.0)");

		testOpt("abs(-9)", null, "abs(-9.0)");
	 ...
	 *            正しい結果
	 */
	private void testOpt(String str, Map<String, Object> map, double expected) {
		...
		opt.optimize(var, new DoubleOperator());
		double ret = Double.parseDouble(opt.toString());
	...
	public void testOpt(String str, Map<String, Object> map) {
		...
		try {
			exp.optimize(var, new DoubleOperator());
			System.out.println("= " + exp.toString());
