...
public class ExpressionObjectTest {
	...
	public void testCond() {
		testEval("2==2", null, null, Boolean.TRUE);
		testEval("2==3", null, null, Boolean.FALSE);
		testEval("2!=3", null, null, Boolean.TRUE);
		testEval("3!=3", null, null, Boolean.FALSE);
		testEval("2<3", null, null, Boolean.TRUE);
		testEval("3<3", null, null, Boolean.FALSE);
		testEval("2<=3", null, null, Boolean.TRUE);
		testEval("3<=3", null, null, Boolean.TRUE);
		testEval("3<=2", null, null, Boolean.FALSE);
		testEval("4>3", null, null, Boolean.TRUE);
		testEval("4>4", null, null, Boolean.FALSE);
		testEval("4>=3", null, null, Boolean.TRUE);
		testEval("4>=4", null, null, Boolean.TRUE);
		testEval("3>=4", null, null, Boolean.FALSE);
		testEval("!0", null, null, Boolean.TRUE);
		testEval("!1", null, null, Boolean.FALSE);
		testEval("!2", null, null, Boolean.FALSE);
		testEval("!!0", null, null, Boolean.FALSE);
	}
	...
	public void testArray() {
		...
		testEval("c=a, c[1]++", map, null, 11);
		Long[] c = (Long[]) map.get("c");
		assertEquals(new Long(12), c[1]);
		assertEquals(new Long(12), a[1]);
	...
	public void testMap() {
		...
		varMap.put("map", m);
		varMap.put("var1", "key1");
		testEval("map[\"key1\"]", varMap, null, 123);
		testEval("map[\"key2\"]", varMap, null, 456);
		testEval("map[var1]", varMap, null, 123);
		testEval("map[ \"key\"+1 ]", varMap, null, 123);
	}
	...
	public void testFunction() {
		testEval("test()", null, null, null);
		testParse("test((1,1+1)");
		testParse("test(1,1+1))");
		Function func = new InvokeFunction();
		testEval("zzz()+foo(1)+test(2,3+4,5*6-7)", null, func, null);

		Map<String, Object> map = new HashMap<String, Object>();
		map.put("a", new FieldTestA(10));
		testEval("a.getInt()", map, func, 10);

		Function math = new MathFunction();
		testEval("abs(-9)", null, math, 9);
		testEval("max(2,3)", null, math, 3);
		testEval("min(2,3)", null, math, 2);
		// testEval("max(2,3.1)", null, math, new Double(3.1));
		testEval("max(2.0,3.1)", null, math, new Double(3.1));
	...
	public void testメンバー複合() {
		...
		Map<String, FieldTestA[]> map = new HashMap<String, FieldTestA[]>();
		map.put("a", arr);
		Function func = new InvokeFunction();
		testEval("a[1].intValue", map, null, new Integer(123));
		testEval("a[1].getInt()", map, func, new Integer(10));
		testEval("a[1].getLong()", map, func, new Long(10));
		System.out.println("java:" + arr[1].getB()[1].get());
		testEval("a[1].getB()[1].get()", map, func, new Integer(22));
	}
	...
	private void testEval(String str, Map<String, Object> map, Function func,
		...
		assertEquals(expected, ((Number) ret).longValue());
	}

	private void testEval(String str, Map<String, FieldTestA[]> map,
			Function func, Object expected) {
		System.out.println("●" + str + "●");
		Expression exp = ExpRuleFactory.getDefaultRule().parse(str);
		if (map != null) {
