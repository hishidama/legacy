...
public class CDataRule extends MarkupRule {
	...
	protected static final CharType[] START_TYPE = { CharType.TAGO, CharType.EXCL, CharType.BRAO, CharType.STRING,
			CharType.BRAO };
	...
	protected void parseName(NamedMarkup tag) throws IOException {
		...
		loop: for (Char ch = data.readChar(); ch != Char.EOF; ch = data.readChar()) {
			switch (ch.getType()) {
			case BRAO:
			case SPACE:
			case EOL:
