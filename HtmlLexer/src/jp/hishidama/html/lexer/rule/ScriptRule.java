...
public class ScriptRule extends HtLexerRule {
			...
			try {
				...
				if (tagRule.isStart(cs)) {
					if (cs.get(1).getType() == CharType.SLASH) {
						Char n = cs.get(2);
						if (n.getType() == CharType.STRING
								&& "script".equalsIgnoreCase(n.getString())) {
							return true;
						}
					}
				}
	...
	protected StringBuilder parseSQ(ScriptToken text, StringBuilder sb, Char ch)
			throws IOException {
		sb.append(ch.getSB());
		boolean esc = false;
		for (;;) {
	...
	protected StringBuilder parseDQ(ScriptToken text, StringBuilder sb, Char ch)
			throws IOException {
		sb.append(ch.getSB());
		boolean esc = false;
		for (;;) {
